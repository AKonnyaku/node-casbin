...
export async function addCachedEnforcerBenchmarks(suite: benchmark.Suite) {
    ...
    const eResRoles = await newCachedEnforcer(resolve('examples/rbac_with_resource_roles_model.conf'), resolve('examples/rbac_with_resource_roles_policy.csv'));
    ...
    const eDomains = await newCachedEnforcer(resolve('examples/rbac_with_domains_model.conf'), resolve('examples/rbac_with_domains_policy.csv'));
