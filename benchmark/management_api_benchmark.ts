...
export async function addManagementApiBenchmarks(suite: benchmark.Suite) {
    ...
    suite.add('BenchmarkHasPolicySmall', {
        ...
        fn: async (deferred: any) => {
            await eHasSmall.hasPolicy(`user${Math.floor(Math.random() * 100)}`, `data${Math.floor(Math.floor(Math.random() * 100) / 10)}`, 'read');
            deferred.resolve();
        }
    });
    ...
    suite.add('BenchmarkHasPolicyMedium', {
        defer: true,
        fn: async (deferred: any) => {
            await eHasMedium.hasPolicy(`user${Math.floor(Math.random() * 1000)}`, `data${Math.floor(Math.floor(Math.random() * 1000) / 10)}`, 'read');
            deferred.resolve();
        }
    });
    ...
    suite.add('BenchmarkHasPolicyLarge', {
        defer: true,
        fn: async (deferred: any) => {
            await eHasLarge.hasPolicy(`user${Math.floor(Math.random() * 10000)}`, `data${Math.floor(Math.floor(Math.random() * 10000) / 10)}`, 'read');
            deferred.resolve();
        }
    });
