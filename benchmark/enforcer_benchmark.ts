...
export async function addEnforcerBenchmarks(suite: benchmark.Suite) {
    ...
    const eResRoles = await newEnforcer(resolve('examples/rbac_with_resource_roles_model.conf'), resolve('examples/rbac_with_resource_roles_policy.csv'));
    ...
    const ePatternLarge = await newEnforcer(resolve('examples/performance/rbac_with_pattern_large_scale_model.conf'), resolve('examples/performance/rbac_with_pattern_large_scale_policy.csv'));
